...
int first_read()
{
    ...
    unique_ptr<FILE,decltype(deleter)> input (fopen(name_input, "rb"),deleter);
    if (input.get() == NULL)
    {
    ...
    unique_ptr<FILE,decltype(deleter)> output (fopen(name_tmp, "wb"),deleter);
    if (output.get() == NULL)
    {
        printf("Cannot open file.\n");
        ok = false;
    ...
    do
    {
        now = fread(q, sizeof(type), amount, input.get());
        i++;
        std::sort(q, q+now);
        fwrite(q, sizeof(type), now, output.get());
    }
    while (now == amount);
    free(q);
    return i;
...
void mergesort(const int beg, const int size_of_chunk, const char *name_output, FILE *output, bool bol)
{
    if (!bol)
        return ;
    unique_ptr<FILE,decltype(deleter)> input1 (fopen(name_output, "rb"),deleter);
    if (input1.get() == NULL)
    {
        printf("Cannot open file.\n");
        ok = false;
    ...
    unique_ptr<FILE,decltype(deleter)> input2 (fopen(name_output, "rb"),deleter);
    if (input2.get() == NULL)
    {
        printf("Cannot open file.\n");
        ok = false;
    ...
    for (int i = 1; i < beg; i++)
    {
        fread(q1, sizeof(type), amount, input1.get());
        fread(q2, sizeof(type), amount, input2.get());
    }
    ...
    fread(q1, sizeof(type), amount, input1.get());
    int how_read = fread(q2, sizeof(type), amount, input2.get());
    ...
    do
    {
        ...
        {
            i2++;
            how_read = fread(q2, sizeof(type), amount, input2.get());
            r2 = q2;
        }
...
{
    unique_ptr<FILE,decltype(deleter)> output1 (fopen(name_output1, "rb"),deleter);
    if (output1.get() == NULL)
    {
        printf("Cannot open file.\n");
        ok = false;
    ...
    unique_ptr<FILE,decltype(deleter)> output2 (fopen(name_output2, "rb"),deleter);
    if (output2.get() == NULL)
    {
        printf("Cannot open file.\n");
        ok = false;
    ...
    unique_ptr<FILE,decltype(deleter)> output (fopen(name_output, "wb"),deleter);
    if (output.get() == NULL)
    {
        printf("Cannot open file.\n");
        ok = false;
    ...
    if (a)
    {
        while (how_read = fread(q, sizeof(type), amount, output1.get()))
        {
            fwrite(q, sizeof(type), how_read, output.get());
        }
        while (how_read = fread(q, sizeof(type), amount, output2.get()))
        {
            fwrite(q, sizeof(type), how_read, output.get());
        }
    }
...
int main()
{
    ...
    for (int j = 0; j < i; j++)
    {
        unique_ptr<FILE,decltype(deleter)> output1 (fopen(name_output1, "wb"),deleter);
        unique_ptr<FILE,decltype(deleter)> output2 (fopen(name_output2, "wb"),deleter);
        if (output1.get() == NULL)
        {
            printf("Cannot open file.\n");
            return 0;
